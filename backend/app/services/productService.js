...
const getAllProduct = (limit, page, sort, filter, category) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (filter && filter != null) {
                // console.log(filter)
                const allObjectFilter = await Product.find({ ["name"]: { '$regex': filter } }).limit(limit).skip(page * limit).sort({ createdAt: -1, updatedAt: -1 })
                resolve({
                    status: 'OK',
                    message: 'Success',
                    data: allObjectFilter,
